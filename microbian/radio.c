...
static void init_radio()
{
	...
	/* Basic configuration */
	RADIO.PCNF0 =
		FIELD(RADIO_PCNF0_LFLEN, 8); /* One 8-bit length field */
	RADIO.PCNF1 =
		BIT(RADIO_PCNF1_WHITEEN) /* Whitening enabled */
...
static void radio_task(int dummy)
{
	...
	RADIO.INTENSET = BIT(RADIO_INT_READY) | BIT(RADIO_INT_END) |
									 BIT(RADIO_INT_DISABLED);
	connect(RADIO_IRQ);
	enable_irq(RADIO_IRQ);
