...
static const char *state_name[] = { "[DEAD]   ", "[ACTIVE] ",
																		"[SEND]   ", "[RECEIVE]",
																		"[SENDREC]", "[IDLE]   " };
...
static void microbian_dump(void)
{
	...
	for (int pid = 0; pid < os_nprocs; pid++) {
		...
		kprintf_internal("%s%d: %s %x stk=%s %s\r\n", (pid < 10 ? " " : ""),
										 pid, state_name[p->state], (unsigned)p->stack, buf,
										 p->name);
	}
}
...
static const char *exc_name[] = { "*zero*",		 "Reset",			"NMI",
...
static proc create_proc(char *name, unsigned stksize)
{
	...
	for (unsigned *p = (unsigned *)stack; p < sp; p++)
		*p = BLANK;
